...
function getDefault()
{
  ...
  if (!$rollingCurlInstance) {
    $rollingCurlInstance = new RollingCurl();
    ...
    $rollingCurlInstance->options[CURLOPT_MAXREDIRS] = 2;
    $rollingCurlInstance->options[CURLOPT_TIMEOUT] = 15;
    $rollingCurlInstance->options[CURLOPT_USERAGENT] = "WordPress/$wpVersion WooCommerce/$wooVersion (Katalys PHP plugin $pluginVersion)";

    // ensure waiting requests are handled before shutdown
    register_shutdown_function(__NAMESPACE__ . '\\onShutdown');
  }

  return $rollingCurlInstance;
}

/**
 * Await any in-flight curl() requests.
 */
function onShutdown()
{
  /** @var RollingCurl $rollingCurlInstance */
  $rollingCurlInstance = getDefault();
...
class RollingCurl
{
  ...
  public function start()
  {
    ...
    $this->multiTimeout = 0;
    if (!$this->session->valid()) {// run to first "yield" statement
      // no waiting requests!
      trigger_error("No requests processed", E_USER_NOTICE);
    } else {
  ...
  public function waitFor(RollingCurlRequest $req)
  {
    ...
    if (0 < ($i = array_search($req, $this->requests, true))) {
...
class RollingCurlRequest
{
   ...
   * @param array        $options
   */
  public function __construct($url, $method = self::DEFAULT_METHOD, $postData = null, array $headers = null, array $options = null)
  {
    $this->url      = $url;
  ...
  public function __toString()
  {
    return __CLASS__ . '{' . $this->method . ' ' . $this->url
      . ' '
      . (!$this->time ? 'waiting' : ($this->output === null ? 'started' : (isset($this->info['http_code']) ? $this->info['http_code'] : 'done')))
      . '}';
  }
}
