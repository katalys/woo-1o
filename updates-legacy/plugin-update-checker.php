...
/**
 * Plugin Update Checker Library 1.7.1
 * http://w-shadow.com/
 * 
 * Copyright 2014 Janis Elsts
 * Released under the  MIT license. See license.txt for details.
 */
if (!class_exists('FCMPluginUpdateChecker_1_7')) :
	/**
	 ...
	 * @access public
	 */
	/* Modified 01/2019, 08/2021 by Don Fischer <dfischer@fischercreativemedia.com> for custom use */

	class FCMPluginUpdateChecker_1_7
	{
		...
		public $debugMode; 		//Set to TRUE to enable error reporting. Errors are raised using trigger_error() and should be logged to the standard PHP error log.
		public $throttleRedundantChecks = true; //Check less often if we already know that an update is available.
		...
		public function __construct($metadataUrl, $pluginFile, $slug = '', $checkPeriod = 12, $optionName = '', $muPluginFile = '')
		{
			...
			//If no slug is specified, use the name of the main plugin file as the slug. For example, 'my-cool-plugin/cool-plugin.php' becomes 'cool-plugin'.
			if (empty($this->slug)) {
			...
			//Backwards compatibility: If the plugin is a mu-plugin but no $muPluginFile is specified, assume it's the same as $pluginFile given that it's not in a subdirectory (WP only looks in the base dir).
		...
		protected function installHooks()
		{
			...
			if ($this->checkPeriod > 0) {
				//Trigger the check via Cron. Try to use one of the default schedules if possible as it's less likely to conflict with other plugins and their custom schedules.
				...
				//In case Cron is disabled or unreliable, we also manually trigger the periodic checks while the user is browsing the Dashboard. 
		...
		public function requestInfo($queryArgs = array())
		{
			...
			$queryArgs['request'] = urlencode(serialize(array('slug' => $this->slug, 'version' => $queryArgs['installed_version'])));
			$queryArgs = apply_filters('puc_request_info_query_args-' . $this->slug, $queryArgs);
			$queryArgs['action'] = 'plugin_information';
			...
			if (!is_wp_error($result) && isset($result['response']['code']) && ($result['response']['code'] == 200) && !empty($result['body'])) {
				$pluginInfo = FCMPluginInfo_1_7::fromJson($result['body'], $this->debugMode);
				if ($pluginInfo == null)
					return null;
		...
		{
			//For the sake of simplicity, this function just calls requestInfo() and transforms the result accordingly.
			$pluginInfo = $this->requestInfo(array('checking_for_updates' => '1'));
			if ($pluginInfo == null) {
				return null;
			}
			return FCMPluginUpdate_1_7::fromPluginInfo($pluginInfo);
		}
		/**
		...
		public function getInstalledVersion()
		{
			...
			if (!is_file($this->pluginAbsolutePath)) {
				...
				if ($this->debugMode) {
					trigger_error(sprintf("Can't to read the Version header for %s. The file does not exist.", $this->pluginFile), E_USER_WARNING);
				}
				return null;
			}
		...
		public function checkForUpdates()
		{
			...
			if ($installedVersion === null) {
				if ($this->debugMode) {
					trigger_error(sprintf('Skipping update check for %s - installed version unknown.', $this->pluginFile), E_USER_WARNING);
				}
				return null;
			}
		...
		public function maybeCheckForUpdates()
		{
			...
			if (in_array($currentFilter, array('load-update-core.php', 'upgrader_process_complete'))) {
				...
				//WordPress cron schedules are not exact, so lets do an update check even if slightly less than $checkPeriod hours have elapsed since the last check.
				$cronFuzziness = 20 * 60;
		...
		public function getUpdateState()
		{
			...
			if (!empty($state) && isset($state->update) && is_object($state->update)) {
				$state->update = FCMPluginUpdate_1_7::fromObject($state->update);
			}
			return $state;
		}
		/**
		...
		public function injectUpdate($updates)
		{
			...
			//No update notifications for mu-plugins unless explicitly enabled. The MU plugin file is usually different from the main plugin file so the update wouldn't show up properly anyway.
...
if (!class_exists('FCMPluginInfo_1_7')) :
	/**
	...
	class FCMPluginInfo_1_7
	{
		...
		public static function fromJson($json, $triggerErrors = false)
		{
			...
			if (empty($apiResponse) || !is_object($apiResponse)) {
				if ($triggerErrors) {
					trigger_error("Failed to parse plugin metadata. Try validating your .json file with http://jsonlint.com/", E_USER_NOTICE);
				}
				return null;
			}
			...
			if (!$valid) {
				if ($triggerErrors) {
					trigger_error("The plugin metadata file does not contain the required 'name' and/or 'version' keys.", E_USER_NOTICE);
				}
				return null;
			}
			$info = new self();
		...
		public function toWpFormat()
		{
			...
			$sameFormat = array('name', 'slug', 'version', 'requires', 'tested', 'rating', 'upgrade_notice', 'num_ratings', 'downloaded', 'homepage', 'last_updated', 'banners');
...
if (!class_exists('FCMPluginUpdate_1_7')) :
	/**
	...
	class FCMPluginUpdate_1_7
	{
		...
		public static function fromJson($json, $triggerErrors = false)
		{
			...
			$pluginInfo = FCMPluginInfo_1_7::fromJson($json, $triggerErrors);
			if ($pluginInfo != null) {
...
PucFactory::addVersion('PluginUpdateChecker', 'FCMPluginUpdateChecker_1_7', '1.7');
PucFactory::addVersion('PluginUpdate', 'FCMPluginUpdate_1_7', '1.7');
PucFactory::addVersion('PluginInfo', 'FCMPluginInfo_1_7', '1.7');
/**
...
if (!class_exists('PluginUpdateChecker')) {
	class PluginUpdateChecker extends FCMPluginUpdateChecker_1_7
	{
	}
}
if (!class_exists('PluginUpdate')) {
	class PluginUpdate extends FCMPluginUpdate_1_7
	{
	}
}
if (!class_exists('PluginInfo')) {
	class PluginInfo extends FCMPluginInfo_1_7
	{
	}
}
